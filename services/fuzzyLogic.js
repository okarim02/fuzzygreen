...
class fuzzyVariable{
    ...
    declareRules = function() {
        var veryGood_critere1 = this.fzmod.makeNewFuzzyTerm(this.excellent_critere);
        var good_critere1 = this.fzmod.makeNewFuzzyTerm(this.medium_critere);
        var veryBad_critere1 = this.fzmod.makeNewFuzzyTerm(this.bad_critere);

        var veryGood_critere2 = this.fzmod.makeNewFuzzyTerm(this.excellent_requestsNb);
        var good_critere2 = this.fzmod.makeNewFuzzyTerm(this.medium_requestsNb);
        var veryBad_critere2 = this.fzmod.makeNewFuzzyTerm(this.bad_requestsNb);
...
function test(){
    let a = new fuzzyVariable("Size(Kb)","RequestsNb");

}

module.exports.launch = async function launch(data,data2=[common.exampleScrapperData]){
    // test critères : 
    let crit_less = ["Size(Kb)","RequestsNb","DOMsize(nb elem)","CSSNotExt","JSNotExt","ratioimagesResizedInPage"]; // Plus la valeur est bas, plus on est dans l'excellent // critère retiré pour l'instant : "Http2requests"
    ...
    for(let i = 0 ; i < crit_less.length-2;i+=2){
        ...
        const result_act = getSpecificData(data,crit_less[i]); // résultat actuelle
        const result_ap = getSpecificData(data,crit_less[i+1]); // résultat du critère duo
        ...
        let a = fuzzyVariable(crit_less[i],crit_less[i+1]);

        a.critere_module(crit_less[i],result_act.min,result_act.max,result_act.average,result_act.ecart);
        a.critere_module(crit_less[i+1],result_ap.min,result_ap.max,result_ap.average,result_ap.ecart);

        let fuzzyval1 = a.getCrispValue(url_data[crit_less[i]], url_data[crit_less[i+1]]);

        console.log("Fuzzy val 1 : ", fuzzyval1);
        //const fuzzyVal = getFuzzyValue(url_data[i],result);    
        //fuzzyLogic_values[i] = {} 
        //fuzzyLogic_values[i]["fuzzification"] = null;
        //fuzzyLogic_values[i]["other"] = {min: result_act.min , max: result_act.max, moyenne : result_act.average, median: result_act.median};
    }
